...
function function_f8358462()
{
	...
	if(wait_for_flags)
	{
		...
		level flag::wait_till_any(array("power_on"));
	}
	wait(1);
	...
	while(true)
	{
		...
		{
			zombie_minecart = spawnvehicle("zombie_minecart", self.minecart.origin, self.minecart.angles, self.minecart.targetname + "_reverse");
			zombie_minecart.drivepath = 0;
			zombie_minecart.var_66b5ee1a = self.minecart.var_88249cb5;
			zombie_minecart.var_fdb1feb7 = self.minecart.var_629f3738;
			zombie_minecart function_b1da7b23();
			zombie_minecart function_b6cb1d5a("start");
			zombie_minecart ghost();
			function_59dcb5d1(self.minecart, zombie_minecart);
			zombie_minecart playloopsound("evt_minecart_climb_loop");
			self.minecart.away = 0;
			self.minecart function_b6cb1d5a("start");
			zombie_minecart function_4c5d405f(self.minecart.accel);
			zombie_minecart waittill("reached_end_node");
			zombie_minecart function_f16a275(self.minecart.accel, self.minecart.decel);
			function_59dcb5d1(zombie_minecart, self.minecart);
			zombie_minecart stoploopsound();
			self.minecart playsound("evt_spiketrap_warn");
			zombie_minecart delete();
		}
		else
		{
			self.minecart.away = 0;
