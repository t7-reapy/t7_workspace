...
function __main__()
{
	...
	level.napalmzombieroundrequirement = 2;
	...
	level.napalmexplodekillradiusjugs = 150;
...
{
	if ( isDefined( self ) && isDefined( self.animName ) && ( self.animName == "napalm_zombie" ) )
		return 0;
...
{
	zombies_axis = array::get_all_closest( self.origin, getAiSpeciesArray( "axis", "zombie" ), undefined, undefined, level.napalmzombiedamageradius );
...
function special_napalm_spawn()
{
	spawner_list = zm_ai_napalm::get_napalm_spawners();
    location_list = zm_ai_napalm::get_napalm_locations();
    spawner = Array::random(spawner_list);
    location = Array::random(location_list);
    ai = zombie_utility::spawn_zombie(spawner, undefined, location);
    ai.spawn_point_override = location;
