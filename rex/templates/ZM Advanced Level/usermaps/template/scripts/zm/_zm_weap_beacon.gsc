...
#precache( "fx", "dlc5/tomb/fx_tomb_beacon_glow" );
#precache( "fx", "dlc5/tomb/fx_tomb_beacon_trail" );
#precache( "fx", "dlc5/tomb/fx_tomb_beacon_exp" );
...
function __init__()
{	
	clientfield::register( "scriptmover", "play_beacon_fx", VERSION_SHIP, 1, "int" );
	clientfield::register( "scriptmover", "play_artillery_barrage", VERSION_SHIP, 1, "int" );
...
function artillery_barrage_logic( grenade )
{
	...
	while ( i < n_num_missiles )
	{
		...
		self.a_v_land_spots[ i ] = trace[ "v_position" ];
		wait .05;
		i++;
	}
	i = 0;
	while ( i < n_num_missiles )
	{
		self clientfield::set( "play_artillery_barrage", 1 );
