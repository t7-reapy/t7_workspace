...
function private function_b5da43f3()
{
	...
	behaviortreenetworkutility::registerbehaviortreescriptapi("SetFindFleshState", &setfindfleshstate);
	behaviortreenetworkutility::registerbehaviortreescriptapi("ClearFindFleshState", &clearfindfleshstate);
	behaviortreenetworkutility::registerbehaviortreescriptapi("ontankdeathactionstart", &ontankdeathactionstart);
...
function tank_setup()
{
	...
	self thread tank_closest_player_override();

	//self thread tank_get_closest_player_using_paths();
	//self thread function_56848b85();
	//self thread function_ce3464b9();
				...
				else
				{
					...
					if(a_eligible_players.size > 0)
					{
						...
						else
						{
							...
							{
								self.favoriteenemy = tank_get_closest_player_using_paths(self.origin, a_eligible_players);
							}
							else
							{
...
function closest_player_tank(origin, players)
{
	...
	{
		player = tank_get_closest_player_using_paths(origin, players);
	}
	if(isdefined(player))
	{
		return player;
...
function get_path_length_to_tank()
{
...
function tank_closest_player_override(v_zombie_origin, a_players_to_check)
{
	...
	if(!isdefined(self.var_13ed8adf) || self.var_13ed8adf == level.time)
	{
		...
		e_player_to_attack = tank_get_closest_player_using_paths(v_zombie_origin, a_players_to_check);
		a_players = get_players_on_tank(1);
		if(a_players.size > 0)
		{
...
function tank_get_closest_player_using_paths(origin, players)
{
	...
	for(i = 0; i < players.size; i++)
	{
		...
		if(isdefined(player.b_already_on_tank) && player.b_already_on_tank)
		{
			...
			else
			{
				...
				{
					length_to_player = get_path_length_to_tank();
...
function private function_ce3464b9(players)
{
	...
	{
		if(isdefined(player.am_i_valid) && player.am_i_valid && function_d39fc97a(player))
		{
...
function function_d39fc97a(player)
{
	...
	{
		if(issubstr(player.zone_name, "huh"))
		{
			var_de0d565a = 1;
		}
	}
	...
	{
		if(issubstr(self.zone_name, "huh"))
		{
			var_561c025d = 1;
		}
	}
...
{
	return self entity_on_tank() || (isdefined(self.b_climbing_tank) && self.b_climbing_tank);
}

function ontankdeathactionstart(behaviortreeentity)
{
...
{
	self.zombiemoveactioncallback = &setfindfleshstate;
}
...
{
	zombie_in_chamber = is_point_in_chamber(self.origin);
	a_players = getplayers();
	for(i = 0; i < a_players.size; i++)
	{
		...
		player_in_chamber = is_point_in_chamber(a_players[i].origin);
