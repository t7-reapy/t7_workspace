...
function private _give_player_rewards_and_bonuses() // self == player
{
    ...
    if (self.hellround_progress_reward.did_finish_game && self.hellround_progress_reward.did_survive_bad_path)
    {
        ...
        self zm_weapons::weapon_give(GetWeapon(HRRWRD_FINISHED_MAP_AND_SURVIVED_BAD_PATH_WEAPON), false, false, true, true);
        PRINT_HR_DEBUG("Given weapon for overall success.");
    }
    else if (self.hellround_progress_reward.did_finish_game)
    {
        zm_xcdylan93_utils::update_weapon_camo(HRRWRD_FINISH_MAP_WEAPON_CAMO_INDEX, weapon, weapon.altWeapon, false);
        PRINT_HR_DEBUG("Given weapon camo for finish success.");
    }
    else if (self.hellround_progress_reward.did_survive_bad_path)
    {
        zm_xcdylan93_utils::update_weapon_camo(HRRWRD_SURVIVE_BAD_PATH_WEAPON_CAMO_INDEX, weapon, weapon.altWeapon, false);
    ...
    {
        self zm_weapons::weapon_give(GetWeapon(HRRWRD_LOSESTREAK_2_REWARD), false, false, true, true);
...
function private _save_progress() // self == player
{
    ...
    */
    ...
    finished = self.hellround_progress_reward.did_finish_game & HRRWRD_DATA_HAS_FINISHED_MAP_MASK;
    survived = self.hellround_progress_reward.did_survive_bad_path & HRRWRD_DATA_HAS_SURVIVED_BAD_PATH_MASK;
...
function private _reset_players_progress()
{
    foreach(player in GetPlayers())
    {
        ...
        player save_data::set_save_data(HRRWRD_DATA_INDEX, 0);
    }
