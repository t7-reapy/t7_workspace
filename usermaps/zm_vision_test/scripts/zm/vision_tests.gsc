...
function __init__()
{
	...
	foreach(vision in level.visions)
	{
		visionset_mgr::register_info("visionset", vision, VERSION_SHIP, 1, 31, 1, &visionset_mgr::ramp_in_out_thread_per_player, 0);
	}
}
...
function __main__()
{
	...
	{
		player thread rotate_visionset(level.visions);
	}
}
...
function private rotate_visionset(visions) // self == player
{
	...
	while(true)
	{
		foreach(vision in visions)
		{
			...
			visionset_mgr::activate("visionset", vision, self, 0.6, 1, 0.6);
			wait 3;
			visionset_mgr::deactivate("visionset", vision, self);
		}
	}
}
