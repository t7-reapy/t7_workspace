...
#using scripts\zm\trinket_box_scripts\trinket_box_custom; // your custom gsc here <<<<<
...
#namespace trinket_box_main; // <<<<< name of the main gsc

REGISTER_SYSTEM_EX( "trinket_box_main", &__init__, &__main__, undefined ) // <<<<< name of the main gsc
...
function __init__()
{
    ...
    level.custom_magic_box_weapon_wait             = &default_magic_box_weapon_wait;
    ...
    level.pandora_show_func                     = &default_custom_pandora_show_func;
    level.pandora_fx_func                         = &default_custom_pandora_fx_func;
}
...
function __main__()
{
	...
	// Random Weapon Chest
	zm_utility::add_sound( "open_chest2", "open" );
	zm_utility::add_sound( "music_chest2", "zmb_music_box" );
	zm_utility::add_sound( "close_chest2", "close" );
...
function default_custom_pandora_show_func( e_anchor, e_anchor_target, a_pieces )
{	
	str_classname = self.zbarrier.classname;
...
function default_custom_pandora_fx_func()
{
	str_classname = self.zbarrier.classname;
...
function default_magic_box_weapon_wait()
{
	wait .5;
}
