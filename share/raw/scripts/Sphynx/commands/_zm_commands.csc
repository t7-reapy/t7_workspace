...
function autoexec init()
{
    ...
    clientfield::register("world", "fog_cf_update", VERSION_SHIP, 2, "int", &fog_update_cf_callback, !CF_HOST_ONLY, !CF_CALLBACK_ZERO_ON_NEW_ENT);
...
function private fog_update_cf_callback(n_local_client_num, n_old_val, n_fog_index, b_new_ent, b_initial_snap, str_field_name, b_was_time_jump)
{
    self fog_update(n_fog_index);
}

function private fog_update(fog_index) // fog_index € [0; 3]
{
    ...
    for (client_number = 0; client_number < level.localPlayers.size; client_number++)
    {
        ...
        SetLitFogBank(client_number, -1, lit_fog_bank, -1);
    }
}
